...
Т.к. как вариант в будущем для обоих этих кейсов может понадобиться плюс-минус одинаковый функционал. 

"Штатно" не нашлось кейса для RxJS, реализовал пример использования этой библиотеки через rxMethod из NgRX Signal Store с обработкой ошибок. Сделал мок сервис, который производит "запись" данных с различной задержкой, а также на каждый 3-тий запрос бросает ошибку. Смотрите реализацию в /store/app.store.ts. Вывод ошибок идет в консоль.

В результате по тз сделал почти все. Не успел глубже поработать со стилями и внешним видом.
